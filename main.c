...
#define FIELD_SIZE_X 9
#define FIELD_SIZE_Y 9
#define MINES_NR 10
#define EMPTY 0
#define MINE 9
#define EXP_MINE 10
...
int update_view(int x, int y, int command){
    int i,j;
        ...
        if (FIELD_VIEW[x][y]==EMPTY ) {
            for (i=-1; i<=1; i++) {
                for (j=-1; j<=1; j++) {
                    ...
                    else if(FIELD_VIEW[x+i][y+j]!=EMPTY) update_view(x+i, y+j, SELECT);
                }
            }
        }
    }
